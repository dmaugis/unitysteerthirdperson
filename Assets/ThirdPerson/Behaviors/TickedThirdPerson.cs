...
namespace UnitySteer.Behaviors
{
	...
	public abstract class TickedThirdPerson : Vehicle
    {
        ...
        /// </param>
        private bool ApplySteeringForce(float elapsedTime)
        {
            // Euler integrate (per frame) velocity into position
            var delta = CalculatePositionDelta(elapsedTime);
			delta = Vector3.Scale(delta, AllowedMovementAxes);
		
			if (character != null)
            {
				// get world relative move vector
				//print("TickedThirdPerson.Move velocity=" + Velocity);
				//character.Move(Vector3.zero, false, false);
				//character.Move(Transform.TransformDirection(Transform.position +delta*10.0f), false, false);
				//character.Move(delta, false, false);
				//agent.SetDestination(Transform.TransformDirection(Transform.position) +delta);
				//agent.SetDestination(Transform.position +delta);
				//agent.SetDestination(delta);
				//if (agent.remainingDistance > agent.stoppingDistance)
				character.Move(delta/elapsedTime, false, false);
				//else
				//	character.Move(Vector3.zero, false, false);
		 ...
		 * rounding errors.
		 */
			//print("TickedThirdPerson.AdjustOrientation ");
        ...
        private void Update()
        {
            if (CanMove)
            {
				if(ApplySteeringForce(Time.deltaTime))
